...
void SystemManager::adjustTimeSettingWidgetPos()
{
    ...
    ui->TimeSetswith_setting->move(102,77);
...
//    listV->addAction(new QAction("hello"));
//    listV->addAction(new QAction("hello1"));
//    listV->addAction(new QAction("hello2"));

    ui->TimeSetcomboBox->setView(new QListView());

//    for (int i=0;i<ui->TimeSetcomboBox->count();i++) {
//        ui->TimeSetcomboBox->model()->setData(ui->TimeSetcomboBox->model()->index(i,0), QSize(160, 28), Qt::SizeHintRole);
...
//    }
    ...
    ui->TimeSetswith_timesave->move(102,403);
...
void SystemManager::createDeviceTableHeader()
{
    ...
    btn5up->setGeometry(58,12,11,7);
    btn5down->setGeometry(58,19,11,7);
...
void SystemManager::adjustSystemOperationWidgetPos()
{
    ...
    ui->Systemoperation_label_reboot->move(ui->Systemoperation_pushbutton_reboot->x() + (ui->Systemoperation_pushbutton_reboot->width() -ui->Systemoperation_label_reboot->width())/2,90);

    ui->Systemoperation_pushbutton_reset->move(161,42);
    ui->Systemoperation_label_reset->move(ui->Systemoperation_pushbutton_reset->x() +(ui->Systemoperation_pushbutton_reset->width()- ui->Systemoperation_label_reset->width())/2 ,90);
}
...
void SystemManager::on_Systemoperation_pushbutton_reboot_clicked()
{

    ConfirmDialog *dialog = new ConfirmDialog(this,tr("Confirm Reboot?"));
    if(dialog->showDialog()){

    }
}

void SystemManager::on_Systemoperation_pushbutton_reset_clicked()
{
    ConfirmDialog *dialog = new ConfirmDialog(this,tr("Confirm Reset?"));
    if(dialog->showDialog()){

    }
}

void SystemManager::on_UserManagerpushButton_submit_clicked()
{
    MessageDialog *dialog = new MessageDialog(this,"输入错误 请重新输入");
